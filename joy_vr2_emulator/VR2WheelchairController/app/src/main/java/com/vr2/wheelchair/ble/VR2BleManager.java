...
public class VR2BleManager extends BleManager {
    ...
    // Context for BroadcastReceiver lifecycle
    ...
    // Receiver for BLE pairing state (ACTION_BOND_STATE_CHANGED)
    // Nordic BleManager 2.7.x does not expose onBondingRequired/onBonded overrides,
    // so we use the standard Android broadcast instead.
    private final BroadcastReceiver bondReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (state == BluetoothDevice.BOND_BONDING) {
                connectionStatus.postValue("Pairing... enter PIN");
                if (callback != null) callback.onBondingRequired();
            } else if (state == BluetoothDevice.BOND_BONDED) {
                connectionStatus.postValue("Paired — connecting...");
                if (callback != null) callback.onBonded();
            } else if (state == BluetoothDevice.BOND_NONE && prev == BluetoothDevice.BOND_BONDING) {
                connectionStatus.postValue("Pairing failed");
                if (callback != null) callback.onError("Pairing failed — check PIN");
            }
        }
    };
    ...
    public interface VR2Callback {
        ...
        // Pairing callbacks (default = no-op, override if needed)
        default void onBondingRequired() {}
        default void onBonded() {}
    }
    ...
    public VR2BleManager(@NonNull Context context) {
        ...
        appContext.registerReceiver(bondReceiver,
                new IntentFilter(BluetoothDevice.ACTION_BOND_STATE_CHANGED));
    }

    /**
     * Call from Activity.onDestroy() to release the BroadcastReceiver.
     */
    public void cleanup() {
        try {
     ...
     *
     * If the device is already bonded (after first pairing), uses autoConnect + no timeout
     * so Android reconnects automatically in background whenever the device is in range.
     *
     * If not yet bonded, gives 30 seconds for the user to complete the pairing PIN dialog.
     */
    public void connectToDevice(@NonNull BluetoothDevice device) {
        boolean alreadyBonded;
        try {
            alreadyBonded = device.getBondState() == BluetoothDevice.BOND_BONDED;
        } catch (SecurityException e) {
            // BLUETOOTH_CONNECT permission not yet granted — treat as unbound
            alreadyBonded = false;
        }

        Log.i(TAG, "connectToDevice — bonded=" + alreadyBonded);
        connectionStatus.postValue(alreadyBonded ? "Reconnecting..." : "Connecting...");

        connect(device)
                // Bonded: autoConnect lets Android reconnect in background, no hard timeout
                // Unbound: manual scan-based connect, 30s to allow PIN entry
                .useAutoConnect(alreadyBonded)
                .timeout(alreadyBonded ? 0 : 30000)
                // Fewer retries when pairing — retrying too fast confuses the pairing state machine
                .retry(alreadyBonded ? 5 : 2, alreadyBonded ? 500 : 1500)
                .done(d -> Log.i(TAG, "Connected to " + d.getAddress()))
                .fail((d, status) -> {
